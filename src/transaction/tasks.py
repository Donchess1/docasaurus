...
def send_lock_funds_merchant_buyer_email(email, values):
    EmailClientV2.send_lock_funds_merchant_buyer_email(email, values)
...
def send_lock_funds_merchant_seller_email(email, values):
    EmailClientV2.send_lock_funds_merchant_seller_email(email, values)


@shared_task
def send_lock_funds_merchant_email(email, values):
    EmailClientV2.send_lock_funds_merchant_email(email, values)
